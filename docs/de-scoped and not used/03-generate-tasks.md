# Rule: Generating a Task List from a PRD and TRD

## Goal

To guide an AI assistant in creating a detailed, step-by-step task list in Markdown format based on an existing Product Requirements Document (PRD) and Technical Requirements Document (TRD). The task list should guide a developer through implementation, following a TDD approach.
...
10.  **Save Task List:** Save the generated document in the `/docs/` directory with the filename `tasks-[prd-file-name].md`, where `[prd-file-name]` matches the base name of the input PRD file (e.g., if the input was `prd-user-profile-editing.md`, the output is `tasks-prd-user-profile-editing.md`).
...
- Unit tests should typically be placed alongside the code files they are testing in the same directory.
